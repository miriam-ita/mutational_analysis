...
tile_data <- read.csv(here("NGS/data", "mutations_MASISACHI.csv"), na.strings = "", sep = ";",
...
patient_details <- read.csv(here("NGS/data", "patient_metadata_MASISACHI.csv"), sep = ";",
...
matrix <-  read.csv(here("NGS/data", "mutations_MASISACHI.csv"), sep = ";",
...
row_order <- read.csv(here("NGS/data", "row_order_oncoprint.csv"), sep = ";",
                            na.strings = "", stringsAsFactors = FALSE) %>% 
  mutate(Pathway = factor(Pathway, levels = c("Pancreatic cancer", 
...
cyto_cols = c("#1a9641", "#f03b20", "#404040")
...
ha_bottom = HeatmapAnnotation(
  ...
  # Histo_Sequencing = patient_details$Histology,
  ...
  col = list(Group = treat_cols, Histo_Cyto = cyto_cols, 
             # Histo_Sequencing = histo_cols,
             DFS = DFS_cols, OS = OS_cols, Grade = grade_cols, Margin = resect_cols,
             Tumour = tumour_cols, Nodes = nodes_cols),
  na_col = "#E9E9E9",
  show_legend = FALSE, gp = gpar(col = "white"),
  annotation_name_gp = gpar(fontsize = 8), gap = unit(0.5,"mm"))

ha_bottom_short = HeatmapAnnotation(
  ...
  # Nodes = patient_details$Nodes,
  show_annotation_name = TRUE,
  col = list(NPL_ratio = NPL_ratio_cols,Group = treat_cols, 
             Histo_Cyto = cyto_cols, 
             # Histo_Sequencing = histo_cols,
...
Annotation_legends_short <- list(
  Legend(title = "Somatic events", at = mut_order, labels = mut_labels,
         legend_gp = gpar(fill = mut_fills), labels_gp = gpar(fontsize = 10),
         title_gp = gpar(fontsize = 11, fontface = "bold")),
    Legend(title = "Group of Treatment", labels = treat_labels,
         legend_gp = gpar(fill = treat_cols), labels_gp = gpar(fontsize = 10),
         title_gp = gpar(fontsize = 11, fontface = "bold")),
  Legend(title = "Histo-Cyto \nConcordance", labels = names(cyto_cols), 
         legend_gp = gpar(fill = cyto_cols), labels_gp = gpar(fontsize = 10),
         title_gp = gpar(fontsize = 11, fontface = "bold")),
  # Legend(title = "Hisotlogy sequencing status", labels = names(histo_cols),
  #        legend_gp = gpar(fill = mut_fills), labels_gp = gpar(fontsize = 8),
  #        title_gp = gpar(fontsize = 9, fontface = "bold")),
  ...
  #        title_gp = gpar(fontsize = 9, fontface = "bold")),
  Legend(title = "DFS (months)", col_fun = DFS_cols, at = c(0, 20, 40, 60),
         labels = DFS_cols_names, labels_gp = gpar(fontsize = 10),
         title_gp = gpar(fontsize = 11, fontface = "bold"),
         direction = "horizontal"),
  Legend(title = "OS (months)", col_fun = OS_cols, at = c(0, 20, 40, 60),
         labels = OS_cols_names, labels_gp = gpar(fontsize = 10),
         title_gp = gpar(fontsize = 11, fontface = "bold"),
         direction = "horizontal"),
  Legend(title = "Percentage of \ntumor cells", col_fun = NPL_ratio_cols, 
         at = c(0, 30, 60, 90),
         labels = NPL_ratio_cols_names, labels_gp = gpar(fontsize = 10),
         title_gp = gpar(fontsize = 11, fontface = "bold"),
         direction = "horizontal"))
...
ht_short <- oncoPrint(matrix,
                alter_fun = alter_fun,
                col = mut_fills,
                show_heatmap_legend = FALSE,
                bottom_annotation = ha_bottom_short,
                column_order = column_order,
                column_split = patient_details$Treatment_type,
                column_gap = unit(3, "mm"),
                row_order = 1:nrow(matrix),
                ...
                row_title_rot = 0,
                show_column_names = FALSE) %>% 
  draw(annotation_legend_list = Annotation_legends_short, 
       heatmap_legend_side = "bottom", annotation_legend_side = "bottom")                 
```
...
pdf(here("NGS/figures", "heatmap_MASISACHI.pdf"), width = 30, height = 15)
HM <- draw(ht)
dev.off()

jpeg(here("NGS/figures", "heatmap_MASISACHI.jpeg"), width = 40, height = 25, units = "cm",
     res = 72, type = "windows")
HM <- draw(ht)
dev.off()

pdf(here("NGS/figures", "heatmap_MASISACHI_short.pdf"), width = 30, height = 15)
HM <- draw(ht_short)
dev.off()

jpeg(here("NGS/figures", "heatmap_MASISACHI_short.jpeg"), width = 40, height = 25, units = "cm",
     res = 72, type = "windows")
HM <- draw(ht_short)
dev.off()
```
```{r Only adjuvant group}

pat_det_adj <- patient_details %>% 
  filter(Treatment_group == "2a" | Treatment_group == "2b")

tile_data_long_adj <- tile_data_long %>% 
  filter(Treatment_group == "2a" | Treatment_group == "2b") %>% 
  mutate(Sample_ID = factor(Sample_ID, levels = unique(Sample_ID)))

matrix_adj <- read.csv(here("NGS/data", "mutations_MASISACHI.csv"), sep = ";",
                    stringsAsFactors = FALSE, row.names = 1, header = TRUE, 
                    na.strings = "")
matrix_adj <- matrix_adj %>% 
  select(one_of(as.character(pat_det_adj$Sample_ID)))
matrix_adj = as.matrix(matrix_adj)

column_order_adj <- levels(tile_data_long_adj$Sample_ID)
row_order_adj <- read.csv(here("NGS/data", "row_order_oncoprint.csv"), sep = ";",
                            na.strings = "", stringsAsFactors = FALSE) %>% 
  mutate(Pathway = factor(Pathway, levels = c("Pancreatic cancer", 
                                              "DNA damage repair", "Cell cycle",
                                              "Other oncogenes")))  

treat_cols_adj = c("#d95f02", "#7570b3")
names(treat_cols_adj) = c("2a", "2b")
treat_adj_labels = c("Adjuvant PEXG", "Adjuvant GEM")

ha_bottom_adj = HeatmapAnnotation(
  Group = pat_det_adj$Treatment_group,
  # Histo_Sequencing = patient_details$Histology,
  Histo_Cyto = pat_det_adj$Cytology,
  DFS = pat_det_adj$DFS,
  OS = pat_det_adj$OS,
  Grade = pat_det_adj$Grade,
  Margin = pat_det_adj$Resection_margin,
  Tumour = pat_det_adj$Tumour,
  Nodes = pat_det_adj$Nodes,
  show_annotation_name = TRUE,
  col = list(
    Group = treat_cols_adj,
    Histo_Cyto = cyto_cols,
    # Histo_Sequencing = histo_cols,
             DFS = DFS_cols, OS = OS_cols, Grade = grade_cols, Margin = resect_cols,
             Tumour = tumour_cols, Nodes = nodes_cols),
...
Annotation_legends_adj <- list(
  ...
  #        legend_gp = gpar(fill = mut_fills), labels_gp = gpar(fontsize = 8),
  #        title_gp = gpar(fontsize = 9, fontface = "bold")),
  Legend(title = "Group of Treatment", labels = treat_adj_labels,
         legend_gp = gpar(fill = treat_cols_adj), labels_gp = gpar(fontsize = 8),
         title_gp = gpar(fontsize = 9, fontface = "bold")),
  Legend(title = "Grade", labels = names(grade_cols), 
         legend_gp = gpar(fill = grade_cols), labels_gp = gpar(fontsize = 8),
         title_gp = gpar(fontsize = 9, fontface = "bold")),
...
ht_adj <- oncoPrint(matrix_adj,
                alter_fun = alter_fun,
                col = mut_fills,
                show_heatmap_legend = FALSE,
                bottom_annotation = ha_bottom_adj,
                column_order = column_order_adj,
                # column_split = patient_details$Treatment_type,
                # column_gap = unit(3, "mm"),
                row_order = 1:nrow(matrix_adj),
                row_split = row_order_adj$Pathway,
                row_gap = unit(1, "mm"),
                row_title_gp = gpar(fontsize = 10),
                show_column_names = TRUE,
                remove_empty_rows = TRUE) %>% 
  draw(annotation_legend_list = Annotation_legends_adj)

pdf(here("NGS/figures", "heatmap_MASISACHI_adj.pdf"), width = 30, height = 15)
HM <- draw(ht_adj)
dev.off()

jpeg(here("NGS/figures", "heatmap_MASISACHI_adj.jpeg"), width = 55, height = 30, units = "cm",
     res = 72, type = "windows")
HM <- draw(ht_adj)
dev.off()

```
```{r Only neoadjuvant group,}

pat_det_neo <- patient_details %>% 
  filter(Treatment_group == "1" | Treatment_group == "3" | Treatment_group == "4")

tile_data_long_neo <- tile_data_long %>% 
  filter(Treatment_group == "1" | Treatment_group == "3" | Treatment_group == "4") %>% 
  mutate(Sample_ID = factor(Sample_ID, levels = unique(Sample_ID)))

matrix_neo <- read.csv(here("NGS/data", "mutations_MASISACHI.csv"), sep = ";",
                    stringsAsFactors = FALSE, row.names = 1, header = TRUE, 
                    na.strings = "")
matrix_neo <- matrix_neo %>% 
  select(one_of(as.character(pat_det_neo$Sample_ID)))
matrix_neo = as.matrix(matrix_neo)

column_order_neo <- levels(tile_data_long_neo$Sample_ID)
row_order_neo <- read.csv(here("NGS/data", "row_order_oncoprint.csv"), sep = ";",
                            na.strings = "", stringsAsFactors = FALSE) %>% 
  mutate(Pathway = factor(Pathway, levels = c("Pancreatic cancer", 
                                              "DNA damage repair", "Cell cycle",
                                              "Other oncogenes")))  

treat_cols_neo = c("#1b9e77", "#fb9a99", "#a6cee3")
names(treat_cols_neo) = c("1", "3", "4")
treat_neo_labels = c("Neoadjuvant PEXG", "Neoadjuvant PEXG/PEFG", 
                     "Neoadjuvant PAXG/AG")

ha_bottom_neo = HeatmapAnnotation(
  Group = pat_det_neo$Treatment_group,
  # Histo_Sequencing = patient_details$Histology,
  Histo_Cyto = pat_det_neo$Cytology,
  DFS = pat_det_neo$DFS,
  OS = pat_det_neo$OS,
  Grade = pat_det_neo$Grade,
  Margin = pat_det_neo$Resection_margin,
  Tumour = pat_det_neo$Tumour,
  Nodes = pat_det_neo$Nodes,
  show_annotation_name = TRUE,
  col = list(
    Group = treat_cols_neo,
    Histo_Cyto = cyto_cols,
    # Histo_Sequencing = histo_cols,
             DFS = DFS_cols, OS = OS_cols, Grade = grade_cols, Margin = resect_cols,
             Tumour = tumour_cols, Nodes = nodes_cols),
...
Annotation_legends_neo <- list(
  ...
  #        legend_gp = gpar(fill = mut_fills), labels_gp = gpar(fontsize = 8),
  #        title_gp = gpar(fontsize = 9, fontface = "bold")),
  Legend(title = "Group of Treatment", labels = treat_neo_labels,
         legend_gp = gpar(fill = treat_cols_neo), labels_gp = gpar(fontsize = 8),
         title_gp = gpar(fontsize = 9, fontface = "bold")),
  Legend(title = "Grade", labels = names(grade_cols), 
         legend_gp = gpar(fill = grade_cols), labels_gp = gpar(fontsize = 8),
         title_gp = gpar(fontsize = 9, fontface = "bold")),
...
ht_neo <- oncoPrint(matrix_neo,
                alter_fun = alter_fun,
                col = mut_fills,
                show_heatmap_legend = FALSE,
                bottom_annotation = ha_bottom_neo,
                column_order = column_order_neo,
                # column_split = patient_details$Treatment_type,
                # column_gap = unit(3, "mm"),
                row_order = 1:nrow(matrix_adj),
                row_split = row_order_adj$Pathway,
                ...
                show_column_names = TRUE,
                remove_empty_rows = TRUE) %>% 
    draw(annotation_legend_list = Annotation_legends_neo)

pdf(here("NGS/figures", "heatmap_MASISACHI_neo.pdf"), width = 30, height = 15)
HM <- draw(ht_neo)
dev.off()

jpeg(here("NGS/figures", "heatmap_MASISACHI_neo.jpeg"), width = 55, height = 30, units = "cm",
     res = 72, type = "windows")
HM <- draw(ht_neo)
dev.off()
```
```{r Tentativo per BRCAness survival}
fit <- survfit(Surv(OS, census_os) ~ BRCAness, data= patient_details)
